...
export class ApiService {
  ...
  getTodoList(): Observable<Todo[]> {
    return this.storage.get<Todo[]>(this.storageKey, this.schema).pipe(
  ...
  addTodo(newTodo: Todo): Observable<Todo[]> {
    ...
    return this.getTodoList().pipe(
      ...
      switchMap(updatedTodos => this.setTodoList(updatedTodos).pipe(
        map(() => updatedTodos)
      ))
    );
  }
    ...
    return this.getTodoList().pipe(
      map(todoList => [
        ...todoList?.map(todo => (
          ...
          todo.title === todoData.title ? todoData : todo) ?? [])
      ]),
      switchMap(updatedTodos => this.setTodoList(updatedTodos).pipe(
        map(() => updatedTodos)
      ))
  ...
  deleteTask(todoData: Todo): Observable<Todo[]> {
    return this.getTodoList().pipe(
      map(todoList => todoList?.filter(todo => (
          todo.expirationDate !== todoData.expirationDate ||
          todo.expirationTime !== todoData.expirationTime ||
